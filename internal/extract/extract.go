...
func ExtractFile(path string, content []byte, lang model.Language) (model.ExtractionResult, error) {
	...
	case model.LangR:
		p, err := tsparse.NewParser(tsparse.LangR)
		if err == nil {
			tree, err = p.Parse(content)
			if err != nil {
				e.errors = append(e.errors, model.ExtractionError{
	...
	case model.LangBash:
		p, err := tsparse.NewParser(tsparse.LangBash)
		if err == nil {
			tree, err = p.Parse(content)
			if err != nil {
				e.errors = append(e.errors, model.ExtractionError{
	...
	case model.LangSql:
		p, err := tsparse.NewParser(tsparse.LangSql)
		if err == nil {
			tree, err = p.Parse(content)
			if err != nil {
				e.errors = append(e.errors, model.ExtractionError{
